...
func IsAdmin(ctx *gin.Context) {
	...
	if !exists {
		ctx.JSON(http.StatusNotFound, "tokenString doesn't exist in context")
		ctx.Abort()
		return
	}
	...
	// verify claim by id
	if config.Role != AdminRole {
		newWarn := fmt.Sprintf("Permission denied, only admin role can do this operation")
